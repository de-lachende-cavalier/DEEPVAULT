...
class UserTests(TestCase):
    ...
    def test_user_registration(self):
        ...
        data = {"username": username, "password1": password1, "password2": ""}
        ...
        self.assertEqual(
            len(valid_req.redirect_chain), 1
        )  # aka the redirect to /login/
    ...
    def test_user_login(self):
        ...
        for _ in range(20):
            self.assertFalse(
                self.client.login(
                    username=user_token, password=token_bytes(16).decode("iso-8859-1")
                )
            )
        ...
        for pass_tk in user_token_list:
            if pass_tk == user_token_list[-1]:
                ...
                self.assertTrue(
                    self.client.login(username=user_token, password=pass_tk)
                )
            else:
                self.assertFalse(
                    self.client.login(username=user_token, password=pass_tk)
    ...
    def test_user_deletion(self):
        ...
        user = User.objects.create_user(
            username="test_deletion", password="fulluserwipe1"
        )
