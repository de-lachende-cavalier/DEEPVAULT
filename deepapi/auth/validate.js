...
const ecdh_schema = Joi.object({
  key: Joi.string().hex().length(266).required(),

  counter: Joi.number().integer().min(1).max(10),
});
...
const nonce_schema = Joi.object({
  nonce: Joi.string().hex().length(128).required(),

  counter: Joi.number().integer().min(1).max(10),

  iv: Joi.string().hex().length(32).required(),

  salt: Joi.string().hex().length(64).required(),

  iterations: Joi.number().integer().min(100000).required(),
});
