...
auth_post = requests.post(
    deepapi_host + "/auth", headers=headers, data=json.dumps(ecdh_data)
)
deepapi_public_key = load_pem_public_key(
    auth_post.json()["pub_key"].encode(), default_backend()
)
...
challenge_post = requests.post(
    deepapi_host + "/auth/challenge", headers=headers, data=json.dumps(nonce_data)
)
...
if b"somerandomstuffhere" in deepapi_nonce:
    ...
    symmetric_key = (
        sym_decryptor.update(bytes.fromhex(enc_symmetric_key))
        + sym_decryptor.finalize()
    )
